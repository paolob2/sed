...
class SED(Agent):
    def __init__(self, bound: Vector_Bound, max_iterations: Optional[int] = None, l2_opt: bool = True):
        super().__init__()
        self.bound = bound
    ...
    def best_arm(self, environment: Environment) -> Tuple[Optional[Tuple[int, int]], List[List[int]]]:
        ...
        while True:
            ...
            def disjoint(pairA: Tuple[int, int], pairB: Tuple[int, int]) -> bool:
                return (pairA[0] != pairB[0] and pairA[0] != pairB[1] and
                        pairA[1] != pairB[0] and pairA[1] != pairB[1])

            if self.l2_opt:
                C = [pair for pair in C if ((disjoint(pair[:2], best[:2]) and pair[2] + score_bound > best[2] - score_bound) or
                                            (not disjoint(pair[:2], best[:2]) and pair[2] + score_bound > best[2] - score_bound / 2))]
            else:
            ...
            C = [(pair[0], pair[1], vector_samples[pair[0]].avg.diff(vector_samples[pair[1]].avg)) for pair in C]
